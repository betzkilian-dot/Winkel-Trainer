...
st.markdown(f"""
    ...
    /* Zentriert das Bild */
    .stPlot {{ display: flex; justify-content: center; }}
    </style>
...
# --- Grafik-Erstellung mit Größenparameter ---
def create_plot(task, size_factor):
    # size_factor steuert die figsize (Breite, Höhe)
    fig, ax = plt.subplots(figsize=(size_factor, size_factor * 0.8))
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 8)
    ax.set_aspect('equal')
    ax.axis('off')
    ...
    def draw_angle_mark(pos, is_target=False):
        ...
        ax.text(lx, ly, label, fontsize=20, color=color, fontweight='bold', ha='center', va='center')
...
with st.sidebar:
    ...
    img_size = st.slider("Bildgröße anpassen", min_value=3, max_value=12, value=7, step=1)
    st.info("Vergrößere das Bild für die Anzeige am Beamer oder verkleinere es für Handys.")

col1, col2 = st.columns([1.6, 1], gap="large")

with col1:
    # Graphik anzeigen mit gewählter Größe
    st.pyplot(create_plot(st.session_state.current_task, img_size))
    st.info("Info: Die Geraden g und h sind parallel.")

with col2:
    t = st.session_state.current_task
    st.markdown(f"### Score: `{st.session_state.score} / 10`")
    
    # Gegebener Winkel Box
    st.markdown(f"""
        <div class="given-box">
            Gegeben: {t['given_letter']} = {t['given_val']}°
        </div>
    """, unsafe_allow_html=True)
    
    # Eingabeformular
    with st.form("input_form"):
        user_val = st.number_input(f"Wie groß ist {t['target_letter']}?", min_value=0, max_value=180, step=1)
    ...
    if st.session_state.feedback == "correct":
        ...
        st.error("❌ Leider falsch. Prüfe die Winkelbeziehung!")

    if st.session_state.score >= 10:
        st.balloons()
        st.success("🎉 Ziel erreicht! Du bist ein Winkel-Experte.")
